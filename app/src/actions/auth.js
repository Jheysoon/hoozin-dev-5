...
export const logOutAction = userId => {
  return (dispatch, getStore) => {
    const authSvc = new AuthServiceAPI();
    return authSvc.logOut(userId).then(() => {
      return dispatch({
        type: LOGOUT.INIT
      });
    });
  ...
  return (dispatch, getStore) => {
    ...
    return authSvc
      ...
      .catch(err => {
        ...
        if (
          err.message.search(/password/gi) > 0 &&
          err.message.search(/invalid/gi) > 0
        ) {
          return dispatch({
            type: SIGNUP.ERROR,
            data: { exitCode: 1 }
          ...
          err.message.search(/email/gi) > 0 &&
          err.message.search(/exist/gi) > 0
        ) {
          return dispatch({
            type: SIGNUP.ERROR,
            data: { exitCode: 2 }
